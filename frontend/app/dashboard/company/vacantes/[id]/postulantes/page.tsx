    ...
    <div className="h-[calc(100vh-64px)] flex flex-col bg-[#f7f9fb] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-8 py-5 bg-white border-b border-[#e6e8ea] shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-[#191c1e] font-headline leading-tight">
              {job?.title ?? 'Vacante'}
            </h1>
            <p className="text-sm text-[#737781] mt-0.5">
              <span className="font-semibold text-[#006d37]">{applicants.length}</span>{' '}
              postulante{applicants.length !== 1 ? 's' : ''} en total
              {job?.area ? ` · ${job.area}` : ''}
            </p>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {filtered.length === 0 ? (
              ...
              filtered.map(applicant => {
                ...
                const matched  = applicant.candidate.skills.filter(s =>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <span className="text-[10px] text-[#737781]">
                            {matched}/{job?.skills.length ?? 0} skills
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto">
                <div className="p-8 space-y-6 max-w-3xl mx-auto">
                  ...
                  {/* Evaluación IA — fortalezas y brechas */}
                  {((selected.aiReasons?.length > 0) || (selected.aiGaps?.length > 0)) && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">

                      {selected.aiReasons?.length > 0 && (
                        <div className="bg-[#6bfe9c]/10 border border-[#6bfe9c]/30 rounded-2xl p-5">
                          <div className="flex items-center gap-2 mb-4">
                            <div className="w-7 h-7 rounded-full bg-[#006d37] flex items-center justify-center shrink-0">
                              <CheckCircle2 className="w-4 h-4 text-white" />
                            </div>
                            <h4 className="font-bold text-sm text-[#005228] uppercase tracking-widest">
                              Fortalezas IA
                            </h4>
                          </div>
                          <ul className="space-y-2.5">
                            {selected.aiReasons.map((reason, i) => (
                              <li key={i} className="flex items-start gap-2.5 text-sm text-[#191c1e]">
                                <span className="w-1.5 h-1.5 rounded-full bg-[#006d37] mt-1.5 shrink-0" />
                                {reason}
                              </li>
                            ))}
                          </ul>
                      ...
                      {selected.aiGaps?.length > 0 && (
                        <div className="bg-[#ffdad6]/40 border border-[#ffdad6] rounded-2xl p-5">
                          <div className="flex items-center gap-2 mb-4">
                            <div className="w-7 h-7 rounded-full bg-[#ba1a1a] flex items-center justify-center shrink-0">
                              <XCircle className="w-4 h-4 text-white" />
                            </div>
                            <h4 className="font-bold text-sm text-[#93000a] uppercase tracking-widest">
                              Brechas IA
                            </h4>
                          </div>
                          <ul className="space-y-2.5">
                  ...
                  {selected.candidate.skills.length > 0 && (
                    <div className="bg-white rounded-2xl border border-[#e6e8ea] p-6">
                      <p className="text-xs font-bold uppercase tracking-widest text-[#424750] mb-4">
                        Skills vs. vacante
                      </p>
                      ...
                      <p className="text-xs text-[#737781] mt-3">
                        <span className="font-bold text-[#006d37]">
                          {selected.candidate.skills.filter(s =>
                            job?.skills.map(js => js.toLowerCase()).includes(s.toLowerCase())
                          ).length}
                        </span>
                        {' '}de{' '}
                        <span className="font-bold">{job?.skills.length ?? 0}</span>
                        {' '}skills requeridas coinciden
                      </p>
                    </div>
                  )}
