...
import CreateContractForm from "./_components/CreateContractForm"; // ← import correcto
import {
  FileText, CheckCircle2, AlertCircle,
  ChevronRight, Users, Plus, Calendar,
} from "lucide-react";
...
interface Contract {
  ...
  endDate: string | null;
  totalAmount: number | null;  // ← corregido, era "amount"
  createdAt: string;
...
export default function ContratosEmpresaPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const jobsRes = await api.get("/jobs/company/mine");
      const myJobs: { id: string; title: string }[] = jobsRes.data;

      const results = await Promise.allSettled(
        myJobs.map(job => api.get(`/jobs/${job.id}/applicants`))
      );

      const selectedApps: Application[] = [];
      results.forEach((r, i) => {
        if (r.status === "fulfilled") {
          const applicants = r.value.data as Array<{
            id: string;
            status: string;
            candidate: { id: string; fullName: string | null };
          }>;
          applicants
            ...
            .forEach(a => {
              selectedApps.push({
                ...
                job:       { id: myJobs[i].id, title: myJobs[i].title },
              });
            });
        }
      });
  ...
  // ← Si showForm, renderiza el componente externo en lugar del inline eliminado
  if (showForm) {
    return (
      <CreateContractForm
  ...
  return (
    <div className="max-w-screen-lg mx-auto px-8 py-10">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: "Total contratos",     value: contracts.length, color: "text-[#191c1e]" },
          { label: "Activos",             value: activeCount,      color: "text-[#006d37]" },
          { label: "Esperando candidato", value: pendingCount,     color: "text-[#7c5c00]" },
