...
import {
  ...
  MapPin, Banknote, Wifi, Building2, Sparkles,
} from "lucide-react";
        ...
        {steps.map((s, i) => (
          <div key={s} className="flex items-center flex-1">
            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
              i + 1 < step  ? "bg-[#006d37]"
            : i + 1 === step ? "bg-[#00386c] ring-2 ring-[#00386c]/30"
            : "bg-[#e0e3e5]"}`} />
            {i < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mx-0.5 ${i + 1 < step ? "bg-[#006d37]" : "bg-[#e0e3e5]"}`} />
            )}
          </div>
        ))}
      </div>
...
function ScoreRing({ value, size = 72 }: { value: number; size?: number }) {
  ...
  return (
    <div className="relative flex items-center justify-center flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <span className="absolute text-[11px] font-black" style={{ color }}>{value}%</span>
    </div>
  );
}
...
function SuggestedJobCard({ job, mySkills }: { job: SuggestedJob; mySkills: string[] }) {
  ...
  return (
    <Link href={`/dashboard/candidate/explorar/${job.id}`}
      ...
      <h4 className="font-headline font-bold text-sm text-[#191c1e] leading-snug mb-1 group-hover:text-[#00386c] transition-colors line-clamp-2">
        {job.title}
      </h4>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        <span className="px-2 py-0.5 bg-[#f2f4f6] text-[#424750] rounded-md text-[10px] font-semibold">
...
export default function CandidateDashboardPage() {
  ...
  // Cargar vacantes para sugerencias (sin filtro = todas)
  ...
  // Nombre real del usuario desde su perfil
  ...
  // Habilidades del candidato para calcular match
  const mySkills: string[] = (profileData as { skills?: string[] } | undefined)?.skills ?? [];

  // Sugerencias: top 4 vacantes con mayor match de skills
      ...
      <div className="max-w-screen-2xl mx-auto px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="relative bg-gradient-to-br from-[#00386c] to-[#1a4f8b] rounded-2xl p-5 overflow-hidden group hover:shadow-lg hover:shadow-[#00386c]/20 transition-all duration-300">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="w-9 h-9 rounded-xl bg-white/15 flex items-center justify-center">
                <Send className="w-4 h-4 text-[#a6c8ff]" />
              </div>
              <span className="text-[10px] font-bold text-[#a6c8ff]/70 uppercase tracking-widest">Activo</span>
            </div>
            <p className="text-4xl font-headline font-extrabold text-white leading-none mb-0.5">
              {loading ? "—" : metrics?.activeApplications ?? 0}
            </p>
            <p className="text-[#a6c8ff] text-xs font-semibold mb-3">Postulaciones en curso</p>
            <Link href="/dashboard/candidate/postulaciones"
              className="flex items-center gap-1 text-[11px] text-[#a6c8ff]/70 hover:text-white transition-colors font-bold">
          ...
          <div className="relative bg-white rounded-2xl p-5 border border-[#e6e8ea] overflow-hidden group hover:shadow-lg hover:shadow-[#006d37]/10 hover:border-[#006d37]/20 transition-all duration-300">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="w-9 h-9 rounded-xl bg-[#6bfe9c]/20 flex items-center justify-center">
                <Briefcase className="w-4 h-4 text-[#006d37]" />
              </div>
              <span className="text-[10px] font-bold text-[#737781] uppercase tracking-widest">Contratos</span>
            </div>
            <p className="text-4xl font-headline font-extrabold text-[#191c1e] leading-none mb-0.5">
              {loading ? "—" : metrics?.activeContracts ?? 0}
            </p>
            <p className="text-[#424750] text-xs font-semibold mb-3">Activos ahora mismo</p>
            <Link href="/dashboard/candidate/contratos"
              className="flex items-center gap-1 text-[11px] text-[#737781] hover:text-[#006d37] transition-colors font-bold">
          ...
          <div className="relative bg-[#6bfe9c] rounded-2xl p-5 overflow-hidden group hover:shadow-lg hover:shadow-[#006d37]/20 transition-all duration-300">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="w-9 h-9 rounded-xl bg-[#006d37]/15 flex items-center justify-center">
                <TrendingUp className="w-4 h-4 text-[#005228]" />
              </div>
              <span className="text-[10px] font-bold text-[#005228]/70 uppercase tracking-widest">Mi score</span>
            </div>
            <p className="text-4xl font-headline font-extrabold text-[#00210c] leading-none mb-0.5">
            ...
            <Link href="/profile/candidate"
              className="flex items-center gap-1 text-[11px] text-[#005228]/70 hover:text-[#00210c] transition-colors font-bold">
        ...
        {((metrics?.registeredIncome ?? 0) > 0 || metrics?.avgRatingReceived != null) && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {(metrics?.registeredIncome ?? 0) > 0 && (
              <div className="bg-white rounded-2xl border border-[#e6e8ea] p-5 flex items-center gap-4 hover:shadow-md transition-all">
                ...
                <div>
                  ...
                  <p className="font-extrabold text-[#006d37] text-lg font-headline mt-0.5">
            ...
            {metrics?.avgRatingReceived != null && (
              <div className="bg-white rounded-2xl border border-[#e6e8ea] p-5 flex items-center gap-4 hover:shadow-md transition-all">
                <div className="w-11 h-11 bg-[#a6c8ff]/20 rounded-xl flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-2 mt-0.5">
                    ...
                    <div className="flex gap-0.5">
                      {[1,2,3,4,5].map(i => (
                        <Star key={i} className={`w-3 h-3 ${i <= Math.round(metrics.avgRatingReceived!) ? "text-[#C9A84C] fill-[#C9A84C]" : "text-[#e0e3e5]"}`} />
                      ))}
                    </div>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Link href="/dashboard/candidate/postulaciones"
                className="flex items-center gap-1 text-sm text-[#00386c] font-bold hover:gap-2 transition-all">
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-2xl border border-[#e6e8ea] p-10 text-center">
                <div className="w-14 h-14 bg-[#f2f4f6] rounded-2xl flex items-center justify-center mx-auto mb-3">
                  <Briefcase className="w-6 h-6 text-[#c2c6d1]" />
                </div>
              ...
              <div className="space-y-3">
                {recentApps.slice(0, 4).map(app => (
                  <div key={app.id}
                    className="bg-white rounded-2xl border border-[#e6e8ea] p-4 flex flex-col md:flex-row md:items-center gap-4 hover:border-[#00386c]/20 hover:shadow-md transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        <div className="min-w-0">
                          <h4 className="font-bold text-sm text-[#191c1e] truncate group-hover:text-[#00386c] transition-colors">
                            {app.job.title}
                          </h4>
                          <p className="text-[#737781] text-xs mt-0.5 flex items-center gap-1">
                            <GraduationCap className="w-3 h-3" />
                            {app.job.company?.companyName ?? "Empresa"}
                          </p>
                        </div>
                ...
                {recentApps.length > 4 && (
                  <Link href="/dashboard/candidate/postulaciones"
                    className="flex items-center justify-center gap-2 w-full py-3 bg-white border border-[#e6e8ea] rounded-2xl text-sm font-bold text-[#00386c] hover:border-[#00386c]/30 hover:shadow-sm transition-all">
            ...
            <div className="bg-gradient-to-br from-[#00386c] to-[#1a4f8b] rounded-2xl p-5 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-4">
                  {scoreValue !== null
                    ? <ScoreRing value={scoreValue} size={64} />
                    : <div className="w-16 h-16 flex items-center justify-center text-white/40 text-2xl font-bold">—</div>
                  }
                  <div>
                    <p className="text-white font-headline font-bold text-sm">Fuerza del perfil</p>
                    <p className="text-[#a6c8ff] text-[10px] uppercase tracking-widest font-bold mt-1">
                ...
                <div className="space-y-2">
                  <Link href="/profile/candidate"
                    className="w-full py-2.5 bg-white text-[#00386c] rounded-xl font-bold text-xs hover:bg-[#f2f4f6] transition-colors flex items-center justify-center gap-2">
                    <Pencil className="w-3.5 h-3.5" /> Editar perfil
                  </Link>
                  <Link href="/dashboard/candidate/contratos"
                    className="w-full py-2.5 bg-white/10 border border-white/20 text-white rounded-xl font-bold text-xs hover:bg-white/15 transition-colors flex items-center justify-center gap-2">
                    <FileText className="w-3.5 h-3.5" />
                    Mis contratos
                    {(metrics?.activeContracts ?? 0) > 0 && (
                      <span className="bg-[#6bfe9c] text-[#00210c] text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                ...
                {activeContracts.slice(0, 2).map(c => (
                  <Link key={c.id} href={`/dashboard/candidate/contratos/${c.id}`}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-[#737781] mt-0.5">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-[#737781] mt-0.5">
                  {mySkills.length > 0
                    ? "Vacantes que coinciden con tus habilidades"
                    : "Vacantes disponibles ahora mismo"}
                </p>
              </div>
              <Link href="/dashboard/candidate/explorar"
                className="flex items-center gap-1 text-sm text-[#00386c] font-bold hover:gap-2 transition-all">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
              {suggestedJobs.map(job => (
                <SuggestedJobCard key={job.id} job={job} mySkills={mySkills} />
              ))}
            </div>
          </div>
        )}
          ...
          <div className="relative flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Eye className="w-4 h-4 text-[#6bfe9c]" />
                <span className="text-[#a6c8ff] text-xs font-bold uppercase tracking-widest">Oportunidades</span>
              </div>
