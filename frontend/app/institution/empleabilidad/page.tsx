      ...
      {items.map(i => {
        ...
        return (
          <div key={i.month} className="flex-1 flex flex-col items-center gap-1 min-w-0">
            <span className="text-[9px] font-bold text-[#737781]">{i.count || ""}</span>
            <div className={`w-full rounded-t-md ${accent}`} style={{ height: `${h}%` }} />
            <span className="text-[8px] text-[#737781] truncate w-full text-center">
              {formatMonth(i.month)}
            </span>
          </div>
        );
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-8 py-10 space-y-8">
      ...
      {data?.avgGraduateScore != null && (
        <div className="bg-white rounded-2xl border border-[#e6e8ea] p-6 flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-[#a6c8ff]/20 flex items-center justify-center">
      ...
      <div className="bg-white rounded-2xl border border-[#e6e8ea] p-6 overflow-x-auto">
            ...
            <thead>
              <tr className="border-b border-[#e6e8ea]">
                {["Carrera", "Vinculados", "Contratados", "Tasa inserción"].map(h => (
                  <th key={h} className="text-left px-3 py-2 text-xs font-black uppercase text-[#737781]">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(data?.byCareer ?? []).map(c => (
                <tr key={c.career} className="border-b border-[#f2f4f6]">
                  <td className="px-3 py-3 font-semibold">{c.career}</td>
                  <td className="px-3 py-3">{c.linked}</td>
                  <td className="px-3 py-3">{c.withCompletedContract}</td>
                  <td className="px-3 py-3 font-bold text-[#006d37]">{c.insertionRatePercent.toFixed(1)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
              ...
              return (
                <div key={a.area}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-semibold text-[#191c1e]">{a.area}</span>
                    <span className="text-xs font-bold text-[#00386c]">{a.count}</span>
                  </div>
                  <div className="h-1.5 bg-[#f2f4f6] rounded-full overflow-hidden">
                    <div className="h-full bg-[#00386c] rounded-full" style={{ width: `${pct}%` }} />
                  </div>
                </div>
              );
            })}
