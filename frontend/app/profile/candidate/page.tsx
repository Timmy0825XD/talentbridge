...
import {
  ...
  Loader2, ChevronRight, Bell, BellOff, Menu,
} from "lucide-react";
  ...
  return (
    <div ref={ref} className="relative">
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => { if (value.trim()) { onAdd(value.trim()); setOpen(false); } }}
          className="px-4 py-2.5 bg-[#00386c] text-white rounded-xl font-semibold text-sm hover:bg-[#1a4f8b] transition-colors">
          +
        </button>
      </div>
      {open && (filtered.length > 0 || value.trim()) && (
        <div className="absolute z-30 mt-1.5 w-full bg-white border border-[#e6e8ea] rounded-2xl shadow-xl overflow-hidden">
          {filtered.map(k => (
            <button key={k.id} type="button" onMouseDown={e => e.preventDefault()}
              onClick={() => { onAdd(k.name); onChange(""); setOpen(false); }}
              className="w-full text-left px-4 py-3 text-sm text-[#191c1e] hover:bg-[#f7f9fb] transition-colors flex items-center justify-between">
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb]">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#00386c] to-[#1a4f8b]">
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto px-4 sm:px-8 py-6 sm:py-12">
          <Link href="/dashboard/candidate"
            className="inline-flex items-center gap-2 text-white/60 hover:text-white text-sm font-semibold mb-6 sm:mb-8 transition-colors">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6 sm:gap-8">
            ...
            <div className="relative flex-shrink-0">
              <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-2xl sm:rounded-3xl border-4 border-white/20 overflow-hidden shadow-2xl">
                {form.photoUrl
                  ? <img src={form.photoUrl} alt="Foto de perfil" className="w-full h-full object-cover" />
                  : <div className="w-full h-full bg-white/10 flex items-center justify-center"><User className="w-12 h-12 sm:w-16 sm:h-16 text-white/40" /></div>
                }
              </div>
              <button type="button" onClick={() => photoInputRef.current?.click()}
                className="absolute -bottom-2 -right-2 w-9 h-9 sm:w-10 sm:h-10 bg-[#6bfe9c] text-[#005228] rounded-full flex items-center justify-center shadow-lg hover:bg-[#4ae183] transition-colors">
            ...
            <div className="flex-1 text-center sm:text-left">
              <div className="inline-flex items-center gap-2 bg-[#6bfe9c]/20 text-[#6bfe9c] px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-2">
                {user.role === "STUDENT" ? "Estudiante" : "Egresado"} · Disponible
              </div>
              <h1 className="text-2xl sm:text-4xl md:text-5xl font-black text-white tracking-tight mb-1 font-headline leading-tight">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 mt-4 justify-center sm:justify-start">
                {form.cvUrl && (
                  <a href={form.cvUrl} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 bg-[#6bfe9c] text-[#005228] px-4 sm:px-6 py-2 sm:py-2.5 rounded-full font-bold text-xs sm:text-sm hover:opacity-90 transition-all">
                    <FileText className="w-3.5 h-3.5" /> Ver CV
                  </a>
                )}
                <button type="button" onClick={() => photoInputRef.current?.click()} disabled={photoUploading}
                  className="flex items-center gap-2 bg-white/10 text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-full font-bold text-xs sm:text-sm border border-white/20 hover:bg-white/20 transition-all disabled:opacity-50">
            ...
            <div className="flex-shrink-0 flex flex-col items-center gap-2">
              <div className="relative w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 96 96">
                  ...
                  <circle cx="48" cy="48" r="42" fill="transparent" stroke="#6bfe9c" strokeWidth="6" strokeLinecap="round"
                    strokeDasharray={`${2 * Math.PI * 42}`}
                    strokeDashoffset={`${2 * Math.PI * 42 * (1 - profilePct / 100)}`}
                    style={{ transition: "stroke-dashoffset 0.8s ease" }} />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-xl sm:text-2xl font-black text-white">{profilePct}%</span>
                  <span className="text-[8px] sm:text-[9px] font-bold text-[#6bfe9c] uppercase tracking-tighter">Perfil</span>
                </div>
              </div>
              <p className="text-white/50 text-xs text-center max-w-[100px] sm:max-w-[120px] hidden sm:block">
      ...
      <div className="lg:hidden bg-white border-b border-[#e6e8ea] sticky top-[64px] z-30">
        <button onClick={() => setNavOpen(p => !p)}
          className="w-full flex items-center justify-between px-4 py-3 text-sm font-bold text-[#191c1e]">
          <span className="flex items-center gap-2">
            <Menu className="w-4 h-4 text-[#006d37]" /> Secciones del perfil
          </span>
          <ChevronRight className={`w-4 h-4 text-[#737781] transition-transform ${navOpen ? "rotate-90" : ""}`} />
        </button>
        {navOpen && (
          <div className="border-t border-[#f2f4f6] grid grid-cols-2 gap-0">
            {NAV_SECTIONS.map(({ id, label, icon: Icon }) => (
              <a key={id} href={`#${id}`} onClick={() => setNavOpen(false)}
                className="flex items-center gap-2 px-4 py-3 text-xs font-semibold text-[#424750] hover:bg-[#f7f9fb] hover:text-[#006d37] border-b border-[#f2f4f6] transition-colors">
                <Icon className="w-3.5 h-3.5 text-[#b0b4bc]" />
                {label}
              </a>
            ))}
          </div>
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-8 py-6 sm:py-10 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10">

        {/* ── Sidebar (desktop only) ── */}
        <aside className="hidden lg:block lg:col-span-3 space-y-4 lg:sticky lg:top-6 self-start">
          ...
          <div className="bg-white rounded-2xl border border-[#e6e8ea] p-5 shadow-sm space-y-3">
            ...
            {[
              ...
              <div key={label} className="flex items-center justify-between">
                ...
                <span className={`text-xs font-black px-2 py-0.5 rounded-full ${value > 0 ? "bg-[#6bfe9c]/20 text-[#005228]" : "bg-[#f2f4f6] text-[#c2c6d1]"}`}>{value}</span>
              </div>
            ))}
          </div>

          <InfoCallout title="Entiende tu score" description="Tu puntuación se calcula según habilidades, experiencia, educación y más. Sube tu % completando tu perfil." href="/info/candidatos" linkLabel="Saber más" />
        </aside>

        {/* ── Main content ── */}
        <div className="lg:col-span-9 space-y-5 sm:space-y-6">

          <form id="hero" onSubmit={handleSave} className="space-y-5 sm:space-y-6">

            {/* Información básica */}
            <div className={sectionCard}>
              <div className={sectionHeader}>
                <div className="w-8 h-8 rounded-xl bg-[#00386c] flex items-center justify-center flex-shrink-0">
                  <User className="w-4 h-4 text-white" />
                </div>
                <div>
                  <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Información básica</h2>
              ...
              <div className={`${sectionBody} grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6`}>
                <div className="md:col-span-2">
                  ...
                  <input type="text" required value={form.fullName} onChange={e => set("fullName", e.target.value)} placeholder="e.j. Juan Mario López Guerra" className={inp} />
                </div>
                <div>
                  <label className={lbl}>Teléfono</label>
                  <input type="tel" value={form.phone} onChange={e => set("phone", e.target.value)} placeholder="e.j. 3001234567" className={inp} />
                </div>
                <div className="md:col-span-2">
                  <label className={lbl}>Resumen profesional</label>
                  <textarea rows={4} value={form.summary} onChange={e => set("summary", e.target.value)} placeholder="Cuéntale a las empresas quién eres..." className={`${inp} resize-none`} />
                </div>
              </div>
            </div>

            {/* Información académica */}
            <div id="academic" className={sectionCard}>
              <div className={sectionHeader}>
                <div className="w-8 h-8 rounded-xl bg-[#006d37] flex items-center justify-center flex-shrink-0"><GraduationCap className="w-4 h-4 text-white" /></div>
                <div>
                  <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Información académica</h2>
              ...
              <div className={`${sectionBody} grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6`}>
                ...
                {user.role === "STUDENT" && (
                  <div>
                    ...
                    <input type="number" min={1} max={12} value={form.semester} onChange={e => set("semester", e.target.value)} placeholder="e.j. 7" className={inp} />
                  </div>
                )}
                {user.role === "GRADUATE" && (
                  <div>
                    <label className={lbl}>Año de graduación</label>
                    <input type="number" min={2000} max={new Date().getFullYear()} value={form.graduationYear} onChange={e => set("graduationYear", e.target.value)} placeholder="e.j. 2023" className={inp} />
                  </div>
                )}
              </div>
            </div>
            ...
            <div id="work" className={sectionCard}>
              <div className={sectionHeader}>
                <div className="w-8 h-8 rounded-xl bg-[#00386c] flex items-center justify-center flex-shrink-0"><Briefcase className="w-4 h-4 text-white" /></div>
                <div>
                  <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Preferencias laborales</h2>
              ...
              <div className={`${sectionBody} grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6`}>
                <div>
                  ...
                  <select value={form.workMode} onChange={e => set("workMode", e.target.value as typeof form.workMode)} className={`${inp} cursor-pointer`}>
                ...
                <div>
                  ...
                  <input type="number" min={0} value={form.salaryExpected} onChange={e => set("salaryExpected", e.target.value)} placeholder="e.j. 1500000" className={inp} />
                </div>
              </div>
            </div>

            {/* Habilidades */}
            <div id="skills" className={sectionCard}>
              <div className={sectionHeader}>
                <div className="w-8 h-8 rounded-xl bg-[#006d37] flex items-center justify-center flex-shrink-0"><Wrench className="w-4 h-4 text-white" /></div>
                <div>
                  <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Habilidades</h2>
                  <p className="text-xs text-[#737781]">Escribe para ver sugerencias o agrega nuevas</p>
                </div>
              </div>
              <div className={`${sectionBody} space-y-6 sm:space-y-8`}>
                <div>
                  <label className={lbl}>Habilidades técnicas</label>
                  <SkillInput value={skillInput} onChange={setSkillInput} onAdd={tag => { addSkill(tag, "skills"); setSkillInput(""); }} placeholder="e.j. React, Node.js, Python..." suggestions={techKeywords} />
                  {form.skills.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-4">
                      {form.skills.map(tag => (
                        <span key={tag} className="flex items-center gap-1.5 bg-[#00386c] text-white text-xs font-semibold px-3 py-1.5 rounded-full">
                          {tag}
                          <button type="button" onClick={() => removeSkill(tag, "skills")} className="hover:text-[#ffdad6] transition-colors"><X className="w-3 h-3" /></button>
                        </span>
                      ))}
                    </div>
                  )}
                </div>
                <div className="border-t border-[#f2f4f6] pt-5 sm:pt-6">
                  <label className={lbl}>Habilidades blandas</label>
                  <SkillInput value={softInput} onChange={setSoftInput} onAdd={tag => { addSkill(tag, "softSkills"); setSoftInput(""); }} placeholder="e.j. Trabajo en equipo, Liderazgo..." suggestions={softKeywords} />
                  {form.softSkills.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-4">
                      {form.softSkills.map(tag => (
                        <span key={tag} className="flex items-center gap-1.5 bg-[#6bfe9c]/20 text-[#005228] text-xs font-semibold px-3 py-1.5 rounded-full border border-[#6bfe9c]/40">
                          {tag}
                          <button type="button" onClick={() => removeSkill(tag, "softSkills")} className="hover:text-[#ba1a1a] transition-colors"><X className="w-3 h-3" /></button>
                        </span>
                      ))}
                    </div>
                  )}
            ...
            {/* Idiomas */}
            <div id="langs" className={sectionCard}>
              <div className={`${sectionHeader} justify-between`}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-[#00386c] flex items-center justify-center flex-shrink-0"><Globe className="w-4 h-4 text-white" /></div>
                  <div>
                    <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Idiomas</h2>
                    <p className="text-xs text-[#737781]">Idiomas que manejas y nivel</p>
                  </div>
                </div>
                <button type="button" onClick={addLanguage} className="flex items-center gap-1.5 bg-[#00386c] text-white text-xs font-bold px-3 sm:px-4 py-2 rounded-full hover:bg-[#1a4f8b] transition-colors flex-shrink-0">
                  <Plus className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Agregar</span>
                </button>
              </div>
              <div className={sectionBody}>
                {form.languages.length === 0 ? (
                  <div className="text-center py-8 border-2 border-dashed border-[#e6e8ea] rounded-2xl">
                    <Globe className="w-8 h-8 text-[#c2c6d1] mx-auto mb-3" />
                    <p className="text-sm text-[#737781] font-medium">Agrega los idiomas que manejas</p>
                    <button type="button" onClick={addLanguage} className="mt-3 text-sm font-bold text-[#00386c] hover:underline flex items-center gap-1 mx-auto"><Plus className="w-3.5 h-3.5" /> Agregar idioma</button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {form.languages.map((lang, i) => (
                      <div key={i} className="flex flex-col sm:flex-row sm:items-end gap-3 p-4 bg-[#f7f9fb] rounded-2xl">
                        <div className="flex-1">
                          <label className={lbl}>Idioma</label>
                          <input type="text" value={lang.language} onChange={e => updateLanguage(i, "language", e.target.value)} placeholder="e.j. Inglés..." className={inp} />
                        </div>
                        <div className="sm:w-40">
                          <label className={lbl}>Nivel</label>
                          <select value={lang.level} onChange={e => updateLanguage(i, "level", e.target.value)} className={`${inp} cursor-pointer`}>
                            {LANGUAGE_LEVELS.map(l => <option key={l}>{l}</option>)}
                          </select>
                        </div>
                        <button type="button" onClick={() => removeLanguage(i)} className="self-end p-2.5 text-[#737781] hover:text-[#ba1a1a] hover:bg-[#ffdad6] rounded-xl transition-colors flex-shrink-0"><X className="w-4 h-4" /></button>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {/* Certificaciones */}
            <div id="certs" className={sectionCard}>
              <div className={`${sectionHeader} justify-between`}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-[#006d37] flex items-center justify-center flex-shrink-0"><Award className="w-4 h-4 text-white" /></div>
                  <div>
                    <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Certificaciones</h2>
                    <p className="text-xs text-[#737781]">Cursos, diplomas y certificados</p>
                  </div>
                </div>
                <button type="button" onClick={addCert} className="flex items-center gap-1.5 bg-[#006d37] text-white text-xs font-bold px-3 sm:px-4 py-2 rounded-full hover:bg-[#00743a] transition-colors flex-shrink-0">
                  <Plus className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Agregar</span>
                </button>
              </div>
              <div className={sectionBody}>
                {form.certifications.length === 0 ? (
                  <div className="text-center py-8 border-2 border-dashed border-[#e6e8ea] rounded-2xl">
                    <Award className="w-8 h-8 text-[#c2c6d1] mx-auto mb-3" />
                    <p className="text-sm text-[#737781] font-medium">Agrega tus certificados y diplomas</p>
                    <button type="button" onClick={addCert} className="mt-3 text-sm font-bold text-[#006d37] hover:underline flex items-center gap-1 mx-auto"><Plus className="w-3.5 h-3.5" /> Agregar certificación</button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {form.certifications.map((cert, i) => (
                      <div key={i} className="p-4 sm:p-5 bg-[#f7f9fb] rounded-2xl relative">
                        <button type="button" onClick={() => removeCert(i)} className="absolute top-3 right-3 p-1.5 text-[#737781] hover:text-[#ba1a1a] hover:bg-[#ffdad6] rounded-xl transition-colors"><X className="w-4 h-4" /></button>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pr-8">
                          <div className="sm:col-span-2">
                            <label className={lbl}>Nombre del certificado</label>
                            <input type="text" value={cert.name} onChange={e => updateCert(i, "name", e.target.value)} placeholder="e.j. AWS Certified Developer" className={inp} />
                          </div>
                          <div>
                            <label className={lbl}>Año</label>
                            <input type="number" min={2000} max={new Date().getFullYear()} value={cert.year} onChange={e => updateCert(i, "year", e.target.value)} placeholder="2023" className={inp} />
                          </div>
                          <div className="sm:col-span-3">
                            <label className={lbl}>Institución</label>
                            <input type="text" value={cert.issuer} onChange={e => updateCert(i, "issuer", e.target.value)} placeholder="e.j. Coursera, AWS, Google..." className={inp} />
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            {/* Proyectos */}
            <div id="projects" className={sectionCard}>
              <div className={`${sectionHeader} justify-between`}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-[#00386c] flex items-center justify-center flex-shrink-0"><FolderGit2 className="w-4 h-4 text-white" /></div>
                  <div>
                    <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Proyectos destacados</h2>
                    <p className="text-xs text-[#737781]">Muestra lo que has construido</p>
                  </div>
                </div>
                <button type="button" onClick={addProject} className="flex items-center gap-1.5 bg-[#00386c] text-white text-xs font-bold px-3 sm:px-4 py-2 rounded-full hover:bg-[#1a4f8b] transition-colors flex-shrink-0">
                  <Plus className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Agregar</span>
                </button>
              </div>
              <div className={sectionBody}>
                {form.projects.length === 0 ? (
                  <div className="text-center py-8 border-2 border-dashed border-[#e6e8ea] rounded-2xl">
                    <FolderGit2 className="w-8 h-8 text-[#c2c6d1] mx-auto mb-3" />
                    <p className="text-sm text-[#737781] font-medium">Muestra tus proyectos destacados</p>
                    <button type="button" onClick={addProject} className="mt-3 text-sm font-bold text-[#00386c] hover:underline flex items-center gap-1 mx-auto"><Plus className="w-3.5 h-3.5" /> Agregar proyecto</button>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                    {form.projects.map((proj, i) => (
                      <div key={i} className="group bg-[#f7f9fb] hover:bg-white hover:shadow-lg rounded-2xl p-4 sm:p-5 relative transition-all border border-transparent hover:border-[#e6e8ea]">
                        <button type="button" onClick={() => removeProject(i)} className="absolute top-3 right-3 p-1.5 text-[#737781] hover:text-[#ba1a1a] hover:bg-[#ffdad6] rounded-xl transition-colors opacity-100 sm:opacity-0 sm:group-hover:opacity-100"><X className="w-4 h-4" /></button>
                        <div className="space-y-3 pr-8">
                          <div>
                            <label className={lbl}>Nombre del proyecto</label>
                            <input type="text" value={proj.name} onChange={e => updateProject(i, "name", e.target.value)} placeholder="e.j. TalentBridge..." className={inp} />
                          </div>
                          <div>
                            <label className={lbl}>Descripción</label>
                            <textarea rows={2} value={proj.description} onChange={e => updateProject(i, "description", e.target.value)} placeholder="¿Qué hace? ¿Qué tecnologías usaste?" className={`${inp} resize-none`} />
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <div className="flex justify-end pt-2">
              <button type="submit" disabled={saving}
                className="w-full sm:w-auto bg-gradient-to-br from-[#00386c] to-[#1a4f8b] text-white px-8 sm:px-12 py-4 rounded-full font-black text-sm uppercase tracking-widest shadow-xl shadow-[#00386c]/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-3">
          ...
          {/* Hoja de vida */}
          <div id="cv" className={sectionCard}>
            <div className={sectionHeader}>
              <div className="w-8 h-8 rounded-xl bg-[#006d37] flex items-center justify-center flex-shrink-0"><FileText className="w-4 h-4 text-white" /></div>
              <div>
                <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Hoja de Vida</h2>
                <p className="text-xs text-[#737781]">Solo PDF · máx 5MB</p>
              </div>
            </div>
            <div className={sectionBody}>
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
                <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
                  className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-br from-[#006d37] to-[#00743a] text-white font-bold text-sm px-6 sm:px-7 py-3.5 rounded-full shadow-lg hover:opacity-90 active:scale-95 transition-all disabled:opacity-50">
                  {uploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}
                  {uploading ? "Subiendo..." : "Subir CV"}
                </button>
                {form.cvUrl && (
                  <a href={form.cvUrl} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 text-sm font-bold text-[#006d37] hover:underline px-4 py-3.5 bg-[#6bfe9c]/10 rounded-full">
                    <FileText className="w-4 h-4" /> Ver CV actual
                  </a>
                )}
              </div>
              <input ref={fileInputRef} type="file" accept=".pdf" onChange={handleCvUpload} disabled={uploading} className="hidden" />
            </div>
          </div>

          {/* Notificaciones */}
          <div id="notifs" className={sectionCard}>
            <div className={sectionHeader}>
              <div className="w-8 h-8 rounded-xl bg-[#00386c] flex items-center justify-center flex-shrink-0"><Bell className="w-4 h-4 text-white" /></div>
              <div>
                <h2 className="font-black text-[#191c1e] text-sm sm:text-base font-headline">Notificaciones</h2>
                <p className="text-xs text-[#737781]">Alertas por Telegram de vacantes nuevas</p>
              </div>
            </div>
            <div className={sectionBody}>
              <div className="flex items-center justify-between p-4 sm:p-5 bg-[#f7f9fb] rounded-2xl gap-3">
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center flex-shrink-0 ${form.notificationsEnabled ? "bg-[#6bfe9c]/20" : "bg-[#f2f4f6]"}`}>
                    {form.notificationsEnabled ? <Bell className="w-4 h-4 sm:w-5 sm:h-5 text-[#006d37]" /> : <BellOff className="w-4 h-4 sm:w-5 sm:h-5 text-[#737781]" />}
                  </div>
                  <div className="min-w-0">
                    <p className="font-bold text-sm text-[#191c1e]">Notificaciones por Telegram</p>
                    <p className="text-xs text-[#737781] mt-0.5 truncate">
                      {form.notificationsEnabled ? "Activadas — recibirás alertas" : "Desactivadas"}
                    </p>
                  </div>
                </div>
                <button type="button" onClick={handleToggleNotifications} disabled={togglingNotifs}
                  className={`relative w-14 h-7 rounded-full transition-all duration-300 disabled:opacity-60 flex-shrink-0 ${form.notificationsEnabled ? "bg-[#006d37]" : "bg-[#c2c6d1]"}`}>
                  <span className={`absolute top-0.5 w-6 h-6 bg-white rounded-full shadow-md transition-all duration-300 ${form.notificationsEnabled ? "left-7" : "left-0.5"}`} />
                  {togglingNotifs && <span className="absolute inset-0 flex items-center justify-center"><Loader2 className="w-3.5 h-3.5 text-white animate-spin" /></span>}
                </button>
              </div>
